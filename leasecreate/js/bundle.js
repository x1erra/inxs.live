...
function generateLease(formData, selectedSchedules) {
  ...
  html += '<div class="lease-section"><h2>'+sectionNum+'. IMPORTANT LEGAL INFORMATION</h2><div class="lease-legal"><p>This agreement is governed by the <strong>'+province.legislation+'</strong>.</p><p>Disputes may be resolved through the <strong>'+province.regulator+'</strong>.</p><div style="page-break-inside:avoid;break-inside:avoid;"><h3>Prohibited Clauses in '+province.name+':</h3><ul>'+province.prohibitedClauses.map(function(c){return '<li style="page-break-inside:avoid;break-inside:avoid;">'+c+'</li>';}).join('')+'</ul></div>'+province.notes.map(function(n){return '<p class="lease-info" style="page-break-inside:avoid;break-inside:avoid;">'+n+'</p>';}).join('')+'</div></div>';
  sectionNum++;
